...
fn request(address: &str, method: &str, path: &str, token: Option<&str>, origin: Option<&str>, body: &str) -> String {
...
fn management_requires_token_and_same_origin_and_registers_a_real_folder() {
    ...
    let token = Command::new(env!("CARGO_BIN_EXE_everywhere")).args(["management-token", "--state", state.to_str().unwrap()]).output().unwrap();
    assert!(token.status.success(), "{}", String::from_utf8_lossy(&token.stderr));
    let token = String::from_utf8(token.stdout).unwrap();
    let token = token.trim();
    assert_eq!(token.len(), 64);
    let mut child = Command::new(env!("CARGO_BIN_EXE_everywhere")).args(["manage", "--state", state.to_str().unwrap(), "--listen", "127.0.0.1:0"]).stdout(Stdio::piped()).stderr(Stdio::piped()).spawn().unwrap();
    ...
    let line = rx.recv_timeout(Duration::from_secs(10)).expect("management server did not listen");
    ...
    assert!(request(address, "GET", "/api/status", Some("wrong"), None, "").starts_with("HTTP/1.1 401"));
    assert!(request(address, "GET", "/api/status", Some(token), Some("https://untrusted.example"), "").starts_with("HTTP/1.1 403"));
    let body = serde_json::json!({"action":"create-folder","folder":"photos","root":root,"mode":"bidirectional"}).to_string();
    let result = request(address, "POST", "/api/command", Some(token), Some(&format!("http://{address}")), &body);
