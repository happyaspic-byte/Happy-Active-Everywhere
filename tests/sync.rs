...
impl Node {
    fn new(base: &Path, name: &str) -> Self {
        ...
        ok(&["share-init", "--state", s(&state), "--folder", "personal", "--root", s(&root)]);
        Self { state, root, id }
    }
    fn allow(&self, other: &Self) {
        identity::trust(&self.state, &other.state.join("identity.der")).unwrap();
        ok(&["share-peer", "--state", s(&self.state), "--folder", "personal", "--peer", &other.id]);
    }
    ...
    fn status(&self) -> Value { serde_json::from_str(&self.command("share-status", &[])).unwrap() }
}
struct Server(Child);
...
fn sync(a: &Node, b: &Node) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_everywhere"))
        .args(["sync-serve", "--state", s(&b.state), "--folder", "personal", "--peer", &a.id,
    ...
    let line = rx.recv_timeout(Duration::from_secs(10)).expect("sync server not ready");
    let address = line.trim().strip_prefix("LISTEN ").expect("missing listener address");
    let output = a.command("sync", &["--peer", &b.id, "--addr", address]);
    assert_eq!(serde_json::from_str::<Value>(&output).unwrap()["status"], "complete");
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        if let Some(result) = guard.0.try_wait().unwrap() { assert!(result.success()); break; }
...
fn pair(base: &Path) -> (Node, Node) {
    let a = Node::new(base, "a"); let b = Node::new(base, "b");
    a.allow(&b); b.allow(&a); (a, b)
}
fn sha(bytes: &[u8]) -> String { format!("{:x}", Sha256::digest(bytes)) }
fn head_state(node: &Node, path: &str) -> Value {
    node.status()["entries"].as_array().unwrap().iter().find(|e| e["path"] == path).unwrap()["versions"].clone()
}
...
fn independent_devices_roundtrip_unicode_empty_files_and_directories() {
    ...
    sync(&a, &b);
    assert_eq!(sha(&fs::read(b.root.join("한글 문서/😀.txt")).unwrap()), sha(b"first"));
    ...
    sync(&a, &b);
    assert_eq!(sha(&fs::read(a.root.join("한글 문서/😀.txt")).unwrap()), sha(b"edited on b"));
...
fn offline_concurrent_edits_preserve_both_payloads_without_conflict_growth() {
    ...
    for node in [&a, &b] {
        ...
        let mut contents: Vec<_> = versions.iter().map(|v| sha(&fs::read(v["object_path"].as_str().unwrap()).unwrap())).collect();
        contents.sort();
        let mut expected = vec![sha(b"edited on a"), sha(b"edited on b")]; expected.sort();
...
fn deletion_requires_persistent_approval_before_network_propagation() {
    let tmp = TempDir::new().unwrap(); let (a, b) = pair(tmp.path());
    fs::write(a.root.join("note"), b"keep until approved").unwrap(); sync(&a, &b);
    fs::remove_file(a.root.join("note")).unwrap();
    a.command("share-scan", &[]);
    sync(&a, &b);
    assert_eq!(fs::read(b.root.join("note")).unwrap(), b"keep until approved");
    assert_eq!(a.status()["pending_deletions"][0], "note");
    a.command("share-approve-deletes", &["--all"]);
    sync(&a, &b);
    assert!(!a.root.join("note").exists()); assert!(!b.root.join("note").exists());
