use crate::{
    identity::Identity,
    share::{self, Record, Share},
    transport::{Approval, receive_object, send_object},
    versions::Mode,
    wire::{self, Timing},
};
...
fn approval(
    ...
    Ok(Arc::new(move || {
        ...
        share::check_access(&state, &folder, &peer, writing)
    }))
}
async fn send_metadata<S: AsyncWrite + Unpin>(
    stream: &mut S,
    share: &Share,
    local: &Hello,
    remote: &Hello,
    ...
    if local.mode != Mode::ReceiveOnly && remote.mode != Mode::SendOnly {
        let mut since = if remote.seen_epoch == local.epoch {
            remote.seen_sequence
        } else {
            0
        };
        ensure!(
            since <= local.sequence,
            "peer cursor exceeds local sequence; index rollback requires a new epoch"
        );
        loop {
            check()?;
            let page = share.records(since, local.sequence, 32)?;
            if page.is_empty() {
                break;
            }
            since = page.last().unwrap().seq;
            wire::send(stream, Metadata::Page(page), Timing::control()).await?;
        }
    }
    wire::send(stream, Metadata::End, Timing::control()).await
}
async fn read_metadata<S: AsyncRead + Unpin>(
    stream: &mut S,
    share: &Share,
    peer: &str,
    remote: &Hello,
    ...
    loop {
        check()?;
        match wire::read(stream, Timing::control()).await? {
            Metadata::Page(page) => {
                ensure!(
                    !page.is_empty() && page.len() <= 32,
                    "invalid metadata page"
                );
                for record in &page {
                    ensure!(
                        record.seq > last && record.seq <= remote.sequence,
                        "unordered metadata sequence"
                    );
                    last = record.seq;
                }
...
async fn pull_objects<S: AsyncRead + AsyncWrite + Unpin>(
    stream: &mut S,
    share: &Share,
    check: &Approval,
) -> Result<()> {
    loop {
        ...
        wire::send(stream, &missing, Timing::control()).await?;
        if missing.is_empty() {
            return Ok(());
        }
        for hash in missing {
...
async fn serve_objects<S: AsyncRead + AsyncWrite + Unpin>(
    stream: &mut S,
    share: &Share,
    check: &Approval,
) -> Result<()> {
    loop {
        let requested: Vec<String> = wire::read(stream, Timing::control()).await?;
...
async fn session<S: AsyncRead + AsyncWrite + Unpin>(
    ...
    let local = Hello {
        ...
        epoch: share.config.epoch.clone(),
        sequence: share.sequence()?,
        seen_epoch,
        seen_sequence,
        mode: share.config.mode,
    };
    ...
    let peer = options.peer.clone();
    let share = wire::work(stream, timing, move || {
        if share.config.mode != Mode::SendOnly {
            share.commit_incoming(&peer)?;
        }
        Ok(share)
    })
    .await?;
...
pub async fn connect(state: &Path, folder: &str, peer: &str, address: SocketAddr) -> Result<()> {
    ...
    let identity = Identity::new(state, peer)?;
    let timing = Timing::control();
    let tcp = timeout(timing.io, TcpStream::connect(address)).await??;
    tcp.set_nodelay(true)?;
    let mut stream = timeout(
        timing.io,
        TlsConnector::from(identity.client_protocol(PROTOCOL)?)
            .connect("everywhere.local".try_into()?, tcp),
    )
    .await??;
    identity.check(stream.get_ref().1.peer_certificates())?;
    ensure!(
        stream.get_ref().1.alpn_protocol() == Some(PROTOCOL),
        "folder protocol mismatch"
    );
    ...
    let options = Session {
        ...
        folder: folder.into(),
        peer: peer.into(),
        read: approval(state, folder, peer, certs, false)?,
        write: approval(state, folder, peer, certs, true)?,
        client: true,
    };
...
pub async fn serve(
    ...
    loop {
        ...
        let outcome = async {
            let mut stream = timeout(
                timing.io,
                TlsAcceptor::from(identity.server_protocol(PROTOCOL)?).accept(tcp),
            )
            .await??;
            identity.check(stream.get_ref().1.peer_certificates())?;
            ensure!(
        ...
        if let Err(error) = outcome {
            eprintln!("synchronization rejected: {error:#}");
        }
    }
}
