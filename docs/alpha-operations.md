...
보존 파일의 이름에 기록된 원래 내용 해시와 실제 내용이 다르면 복원을 거부한다.
`.everywhere-versions` 안의 손상 격리본(`.corrupt-N`)과 알 수 없는 이름도 거부한다.
일반 사용자 파일을 복원하는 경우에는 과거 해시가 없으므로 현재 파일의 복사 무결성만 검사한다.
Unix 보존본은 `0600`, 내부 보존·복구 디렉터리는 `0700`으로 만든다.
대소문자·유니코드 별칭은 `.everywhere-locks/<파일명>`의 추가 잠금으로 직렬화한다.
동시에 실행하는 수신기는 모두 새 바이너리로 교체해야 하며, 실행 중 잠금 파일을 지우면 안 된다.
구체적인 적용 범위와 재현 명령은 [안전성 보강 기록](safety-hardening.md)을 참조한다.
...
- 로컬 인덱스는 폴더 marker·SQLite·삭제 기록을 사용한다. 네트워크 삭제 전파는 미구현이다.
...
DB 손상 자동 복구·원격 버전 적용은 아직 없다.
버전 벡터의 세 모드·충돌 판정은 라이브러리 테스트 범위이며 실제 양방향 동기화 완료를 의미하지 않는다.

## 주기 재검사 감시

`watch --db <DB> --root <폴더> --device <장치 ID> --interval-ms 1000`

감시 프로세스는 `watching` 상태 이후 변경 이벤트와 오류를 JSON line으로 출력한다.
누락 파일은 자동 삭제 승인 없이 오류로 유지한다. 파일·마운트가 돌아오면 재검사를 계속한다.
검사는 한 번에 하나씩 실행하며 완료 뒤 지정 시간만큼 대기한다. 최소 간격 50ms.
현재 전체 내용을 재해시하는 polling 구현이다. OS 이벤트 감지·증분 스캔과 2초 p95 목표는 미검증이다.
10만/100만 파일에서 짧은 간격 사용은 CPU·디스크 사용량을 높일 수 있다.
