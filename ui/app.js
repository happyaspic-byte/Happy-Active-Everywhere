...
$('trust-form').addEventListener('submit',async(event)=>{event.preventDefault();const form=new FormData(event.target);try{const file=form.get('certificate');if(file.size>8192)throw new Error('이 화면에서는 8 KiB 이하의 공개 인증서를 선택하세요.');const certificate=Array.from(new Uint8Array(await file.arrayBuffer()));const result=await command({action:'trust-peer',certificate,expected:form.get('expected')});$('grant-form').elements.peer.value=result.peer;$('job-form').elements.peer.value=result.peer;message('장치를 승인했습니다. 이어서 공유할 폴더의 접근을 허용하세요.');}catch(error){message(error.message,true);}});
$('grant-form').addEventListener('submit',async(event)=>{event.preventDefault();const form=new FormData(event.target);try{await command({action:'grant',folder:form.get('folder'),peer:form.get('peer'),remove:false});$('job-form').elements.folder.value=form.get('folder');await refresh();message('폴더 접근을 허용했습니다. 백그라운드 작업을 등록해 연결하세요.');}catch(error){message(error.message,true);}});
